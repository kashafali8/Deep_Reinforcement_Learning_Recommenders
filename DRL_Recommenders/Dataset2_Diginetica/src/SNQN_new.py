    ...
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, name='DQNetwork'):
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            self.output2= tf.compat.v1.layers.dense(self.states_hidden, self.item_num,
                                                             activation=None, use_bias=True)  # all ce logits
            print("OUTPUT 2 SHAPE: ", self.output2.get_shape())
            self.phi = self.output2

            self.w_f = tf.compat.v1.layers.dense(self.inputs, self.hidden_size, use_bias = True, activation = None)
            self.phi2 = tf.matmul(self.states_hidden, self.w_f, transpose_b = True)


            self.final_score = (1 - self.lambda_score) * self.phi + self.lambda_score * self.phi2
