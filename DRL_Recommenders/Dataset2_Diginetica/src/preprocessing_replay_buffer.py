...
def parse_args():
    ...
    parser.add_argument(
        "--sess_start", type=int, default=0, help="Generate from the nth session id"
    )
...
if __name__ == "__main__":
    ...
    print(
        f'Finish reading in {time.strftime("%H:%M:%S", time.gmtime(time.time()-start_t))}'
    )
    ...
    if args.random:
        ...
        sampled_session_id = np.random.choice(
            session_ids, size=target_sess_size, replace=False
        )
    else:
    ...
    sampled_sessions["valid_session"] = sampled_sessions.session_id.map(
        sampled_sessions.groupby("session_id")["item_id"].size() > 2
    )
    sampled_sessions = sampled_sessions.loc[sampled_sessions.valid_session].drop(
        "valid_session", axis=1
    )
    sampled_sessions["valid_session"] = sampled_sessions.session_id.map(
        sampled_sessions.groupby("session_id")["item_id"].size() < 50
    )
    sampled_sessions = sampled_sessions.loc[sampled_sessions.valid_session].drop(
        "valid_session", axis=1
    )
    ...
    print(
        f'Popularity finished in {time.strftime("%H:%M:%S", time.gmtime(time.time()-start_t))}'
    )
    ...
    for id in tqdm(ids):
        ...
        for index, row in group.iterrows():
            s = list(history)
            len_state.append(
                STATE_LEN if len(s) >= STATE_LEN else 1 if len(s) == 0 else len(s)
            )
