...
def parse_args():
    ...
    parser.add_argument("--seed", type=int, default=1234, help="Value of random seed.")
...
if __name__ == "__main__":
    ...
    sorted_events.to_csv(
        os.path.join(DATA, "sorted_events.csv"), index=None, header=True
    )
    ...
    train_ids, val_ids, test_ids = np.array_split(
        total_sessions, (split_ratio[:-1].cumsum() * len(total_sessions)).astype(int)
    )
    ...
    if args.format == "csv":
        ...
        replay_buffer.to_csv(
            os.path.join(DATA, "replay_buffer.csv"), index=None, header=True
        )
    # Store data statistics in dictionary and save as pickled dataframe
    dic2 = {"state_size": [STATE_LEN], "item_num": [pad_item]}
